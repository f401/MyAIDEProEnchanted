...
import io.github.zeroaicy.dexlib.rewriter.RewriterDexs.RuleRewriterModule;

public class RewriterDexs{
	...
	public static void main6(String[] args) throws Exception{
		System.out.println("Lcom/aide/f/3;".indexOf('/', "Lcom/aide/".length()));
	}
	public static void main(String[] args) throws Exception{
		...
		List<RewriterRule> rewriterRules = new ArrayList<>();
		for ( String rule : ruleString ){
			RewriterRule rewriterRule = new RewriterRule(rule);

			if ( rewriterRule.isReTypeName() ){
				typeReNameMap.put(rewriterRule.original, rewriterRule.now);
				//System.out.println(rewriterRule.original + "\t" + rewriterRule.now);
				//规则已处理，不用保留
				continue;
			}
		...
		File dexsZipFile = new File("/storage/emulated/0/AppProjects1/.project/ZeroAicy-WearOs/DexLib2-3/data/RewriterDexs/AIDE+底包.zip");
		File dexsZipOutFile = new File("/storage/emulated/0/AppProjects1/.project/ZeroAicy-WearOs/DexLib2-3/data/RewriterDexs/AIDE+修改.zip");

		RewriterDexs.RuleRewriterModule ruleRewriterModule = new RuleRewriterModule(typeReNameMap, rewriterRules);
		...
		MultiRewriteDexContainer rewriteDexContainer = new MultiRewriteDexContainer();
		for ( String dexEntryName : loadDexContainer.getDexEntryNames() ){
			...
			System.out.println("重写 " + dexEntryName);
		...
		writeTo(dexsZipOutFile, rewriteDexContainer);
		...
		PrintStream printStream = new PrintStream("/storage/emulated/0/AppProjects1/.project/ZeroAicy-WearOs/DexLib2-3/data/RewriterDexs/rule_修改输出.txt");
		...
		printStream.close();
		ruleRewriterModule.log.close();
		System.out.println("rule_修改输出.txt 写入完成");
	}

	

	public static class MultiRewriteDexContainer implements MultiDexContainer<DexFile>{
		private List<String> dexEntryNames = new ArrayList<String>();
		private Map<String, MultiDexContainer.DexEntry<DexFile>> entryMap = new HashMap<>();

		@Override
		public List<String> getDexEntryNames() throws IOException{
			return dexEntryNames;
		}
		public void putEntry(MultiDexContainer.DexEntry<DexFile> dexEntry) throws IOException{
			String entryName = dexEntry.getEntryName();
			entryMap.put(entryName, dexEntry);
			dexEntryNames.add(entryName);
		}
		public void putEntry(final String entryName, final DexFile dexFile) throws IOException{
			dexEntryNames.add(entryName);
			entryMap.put(entryName, new MultiDexContainer.DexEntry<DexFile>(){
					@Override
					...
					public MultiDexContainer<?> getContainer(){
						return MultiRewriteDexContainer.this;
					}
				});
		}
	...
	private static void writeTo(File dexsZipOutFile, RewriterDexs.MultiRewriteDexContainer rewriteDexContainer) throws IOException{
		FileOutputStream fileOutputStream = new FileOutputStream(dexsZipOutFile);
	...
	public static class RuleRewriterModule extends RewriterModule{
		
		public class RuleTypeRewriter extends TypeRewriter{
			@Nonnull
			protected String rewriteUnwrappedType(@Nonnull String value){
				...
				//先从[全局类名重命名Map]中查询
				String renameValue = typeReNameMap.get(value);
				if ( renameValue != null ){
					return renameValue;
				}
				//判断是否是内部类
				{
					...
					indexOf = value.indexOf('$', indexOf);
					if ( indexOf > 0 ){
						//是一个内部类
						//查找外部类
						String external = value.substring(0, indexOf);
						String externalNow = typeReNameMap.get(external + ";");
						if( externalNow != null){
							externalNow = externalNow.substring(0, externalNow.length() - 1);
							String valueNow = externalNow + value.substring(indexOf);
							typeReNameMap.put(value, valueNow);
							println(value + " -> " + valueNow);
							return valueNow;
						}
					}
				}
				...
				//从剩下的规则进行处理
				for ( RewriterRule rule : rewriterRules ){
					if ( rule.isRePackageName() 
						...
						if ( rule.isRePackageName() && value.indexOf('/', original.length() + 1) >= 0 ){
							//不匹配子包
							continue;
						}
						renameValue = rule.now + value.substring(original.length());

						typeReNameMap.put(value, renameValue);
		...
		List<RewriterRule> rewriterRules;
		PrintStream log;
		public RuleRewriterModule(Map<String, String> typeReNameMap, List<RewriterRule> rewriterRules){
			this.typeReNameMap = typeReNameMap;
			this.rewriterRules = rewriterRules;
			
			try{
				//测试
				log = new PrintStream("/storage/emulated/0/Myfiles/work/测试日志.txt");
			}
		...
		public Rewriter<String> getTypeRewriter(Rewriters rewriters){
			return new RuleTypeRewriter();
		}
		


		private void println(String valueNow){
			log.println(valueNow);
		}
	...
	private static void initRule(List<String> ruleString){
		ruleString.add("RePackageName\t\tLcom/aide/unknown/");
		//ruleString.add("RePackageNameAll\tcom/aide/ui\tcom/aides/ui");
